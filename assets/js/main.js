const btnCriar = document.querySelector('button#btn-criar');
btnCriar.addEventListener('click', function() {
    screenComplete();
});

const btnEditar = document.querySelector('button#btn-editar');
btnEditar.addEventListener('click', function() {
    screenComplete('edit');
});

const btnExcluir = document.querySelector('button#btn-excluir');
...
function excluir() {
    if (activeTool === 'edit' && editOpen === 1) {
        const nome = document.querySelector('input#contactName').value;
        const sobrenome = document.querySelector('input#contactSobrenome').value;
        const tittle = `${nome} ${sobrenome}`;
        const cttIndex = cttList.findIndex(function(ctt) {
            return ctt.tittle === tittle;
          });

          if (cttIndex !== -1) {
            cttList.splice(cttIndex, 1);
            limparScreen();
            porNaLista();
            salvar()
        }
    
    } else if (activeTool === 'create' || editOpen === 0) {
        limparScreen()
    }
}

function salvar() {
    const cttJSON = JSON.stringify(cttList);
    localStorage.setItem('contatos', cttJSON);
}

document.addEventListener('DOMContentLoaded', function() {

    const cttJSON = localStorage.getItem('contatos');
    if (cttJSON !== null) {
        const jsonList = JSON.parse(cttJSON);
        for (let ctt of jsonList) {
        cttList.push(ctt);
        }
    }   
    porNaLista();
